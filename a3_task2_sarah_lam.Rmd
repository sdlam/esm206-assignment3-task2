...
gasol_hookshots <- lakers %>% 
  filter(player == "Pau Gasol", type == "hook") %>% #how would you keep all of Pau Gasol's shots and all hook shots?
  mutate(date_update = ymd(date)) %>% 
  mutate(month = month(date_update))
